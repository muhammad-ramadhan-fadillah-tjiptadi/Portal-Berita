...
@section('content')
    ...
    <div class="container py-4">
        <div class="card shadow-sm form-container">
            ...
            <div class="card-body">
                <form action="{{ route('admin.subcategories.store') }}" method="POST" novalidate>
                    @csrf
                    <div class="form-group">
                        <label for="name" class="form-label">Nama Sub Kategori <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                            name="name" value="{{ old('name') }}" required placeholder="Masukkan nama sub kategori">
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                            rows="3" placeholder="Masukkan deskripsi sub kategori (opsional)">{{ old('description') }}</textarea>
                        @error('description')
                            <span class="error-message">
                                <i class="fas fa-exclamation-triangle me-1"></i>
                                {{ $message }}
                    ...
                    <div class="d-flex justify-content-end gap-2 pt-3 border-top">
                        ...
                        <button type="submit" class="btn-alert-primary">
                            <i class="fas fa-save me-1"></i> Simpan Sub Kategori
                        </button>
                    </div>
                </form>
            </div>
